...
export default function Home() {
  ...
  const update = (id:number, value:string, len:number) => { if (!task) return; setAnswers(prev => ({...prev, [task.task_id]: {...prev[task.task_id], [id]: value.replace(/[^a-zA-Z]/g, "").slice(0,len)}})); if (results[task.task_id]) setResults(prev => { const next = {...prev}; delete next[task.task_id]; return next; }); };
  ...
  if(!task) return null;
  let cursor = 0; const pieces = task.gapped_passage.split(/(_+)/g);
      ...
      <section className="signal-shell">
        ...
        <div className="signal-workspace">
          <section className="question-stage">
            ...
            <p className="signal-passage">{pieces.map((piece,index)=>{ if(!/^_+$/.test(piece)) return <span key={`${piece}-${index}`}>{piece}</span>; const g=gaps[cursor++]; const state=result?(result.correct[g.number]?"ok":"no"):""; return <span className="signal-gap-wrap" key={g.number}><input value={current[g.number]??""} maxLength={g.expected.length} onChange={e=>update(g.number,e.target.value,g.expected.length)} onKeyDown={e=>{if(e.key==="Enter") grade();}} style={{width:`${Math.max(34,g.expected.length*13+18)}px`}} className={`signal-gap ${state}`} aria-label={`第 ${g.number} 空，字首 ${g.prefix}`}/>{result && !result.correct[g.number] && <span className="correction">{g.expected}</span>}</span>;})}</p>
