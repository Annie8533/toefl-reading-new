...
type StoredProgress = { attempted: number; correct: number; wrongByGap: Record<string, number>; today: string; todayGroups: number; dailyDone: boolean; dailyReminderShown: boolean; passageCursor: number; priorityReviewTaskIds: number[]; newSincePriority: number };
...
const blankProgress = (): StoredProgress => ({ attempted: 0, correct: 0, wrongByGap: {}, today: todayKey(), todayGroups: 0, dailyDone: false, dailyReminderShown: false, passageCursor: 0, priorityReviewTaskIds: [], newSincePriority: 0 });
const readProgress = (): StoredProgress => { try { const saved = JSON.parse(localStorage.getItem(STORAGE_KEY) ?? "{}"); const state = { ...blankProgress(), ...saved, dailyReminderShown: saved.dailyReminderShown ?? false, wrongByGap: saved.wrongByGap ?? {}, priorityReviewTaskIds: saved.priorityReviewTaskIds ?? [] }; return state.today === todayKey() ? state : { ...state, today: todayKey(), todayGroups: 0, dailyDone: false, dailyReminderShown: false }; } catch { return blankProgress(); } };
...
export default function Home() {
  ...
  const clearDailyDone = () => { setDailyFinished(false); setShowDailyReminder(false); setProgress(previous => ({ ...previous, dailyDone: false })); };
  ...
  const advanceExamPassage = () => {
    ...
    const shouldShowDailyReminder = nextTodayGroups === 2 && !progress.dailyReminderShown;
    if (shouldShowDailyReminder) setShowDailyReminder(true);
    ...
    if (shouldInsertReview) { const reviewTask = bank.find(item => item.task_id === queuedIds[0]); if (reviewTask) { setPriorityTask(reviewTask); setSeed(nextCursor); setAnswers(previous => { const next = { ...previous }; delete next[reviewTask.task_id]; return next; }); setResults(previous => { const next = { ...previous }; delete next[reviewTask.task_id]; return next; }); setProgress(previous => ({ ...previous, passageCursor: nextCursor, newSincePriority: 0, today: todayKey(), todayGroups: nextTodayGroups, dailyReminderShown: previous.dailyReminderShown || shouldShowDailyReminder })); return; } }
    setSeed(nextCursor); setProgress(previous => ({ ...previous, passageCursor: nextCursor, newSincePriority: previous.newSincePriority + 1, today: todayKey(), todayGroups: nextTodayGroups, dailyReminderShown: previous.dailyReminderShown || shouldShowDailyReminder })); if (active < session.length - 1) { setActive(index => index + 1); return; } setScreen("summary");
  };
...
function DailyChoice({ today, hasPriority, finished, onContinue, onPriority, onFinish }: { today: number; hasPriority: boolean; finished: boolean; onContinue: () => void; onPriority: () => void; onFinish: () => void }) { return <section className="daily-choice"><p className="signal-kicker">DAILY STUDY CHECKPOINT</p><h2>{finished ? <>今日練習 <em>已結束。</em></> : <>今日建議的 <em>2 組</em> 已完成。</>}</h2><p>{finished ? "你已選擇今天先到這裡。下次開啟時，會從下一題組繼續。" : `你已完成 ${today}/2 組段落練習。是否繼續？你可以自行決定，不會自動進入下一題。`}</p>{!finished && <div className="daily-choice-actions"><button onClick={onContinue} className="signal-button primary"><ArrowRight size={17}/> CONTINUE NEW PASSAGE／繼續新題</button>{hasPriority && <button onClick={onPriority} className="signal-button next choice-explainer"><RotateCcw size={17}/><span><b>複習高頻錯題</b><small>再練習一次今天錯比較多的題組</small></span></button>}<button onClick={onFinish} className="signal-button ghost">TODAY&apos;S DONE／今天先到這裡</button></div>}</section> }
function DailyGoalReminder({ hasPriority, onContinue, onPriority, onFinish }: { hasPriority: boolean; onContinue: () => void; onPriority: () => void; onFinish: () => void }) { return <aside className="daily-goal-reminder" role="status"><div><p className="signal-kicker">DAILY TARGET COMPLETE</p><strong>今日建議的 2 組已完成。</strong><small>這只是提醒；第 3 組已準備好，你可以直接繼續練習。</small></div><div className="daily-reminder-actions"><button onClick={onContinue} className="signal-button primary">繼續第 3 組<ArrowRight size={16}/></button>{hasPriority && <button onClick={onPriority} className="signal-button next">複習高頻錯題</button>}<button onClick={onFinish} className="signal-button ghost">今天先到這裡</button></div></aside> }
function DailyGoalBanner({ completed }: { completed: number }) { const remaining = Math.max(0, 2 - completed); return <aside className="daily-goal-banner" aria-label="每日學習建議"><div><p className="signal-kicker">DAILY TARGET／每日建議</p><strong>今天建議完成 <span>2 組</span></strong><small>{completed === 0 ? "先完成兩段，再由你決定是否繼續。" : `已完成 ${completed}/2 組，還差 ${remaining} 組。`}</small></div><div className="daily-goal-count"><b>{String(Math.min(completed, 2)).padStart(2, "0")}</b><span>/ 02 組</span></div></aside> }
