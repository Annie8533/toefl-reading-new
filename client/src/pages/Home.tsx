...
export default function Home() {
  ...
  const newRound = () => { const next = seed + 1; setSeed(next); setMode("exam"); setScreen("questions"); setSession(makeRound(progress, next)); setActive(0); setAnswers({}); setResults({}); setReviewItems([]); setReviewActive(0); setReviewDraft({}); setReviewResults({}); setRoundCounted(false); };
  ...
  const startTaskReview = () => {
    ...
    setMode("review"); setScreen("questions"); setReviewItems(misses); setReviewActive(0); setReviewDraft({}); setReviewResults({});
  };
  ...
  const nextReview = () => { if (reviewActive < reviewItems.length - 1) { setReviewActive(index => index + 1); return; } setMode("exam"); setReviewItems([]); setReviewActive(0); setReviewDraft({}); setReviewResults({}); advanceExamPassage(); };
...
function FreePracticeReviewStage({ item, index, total, draft, result, onDraft, onGrade, onNext, accuracy, attempted, reviewCount, today }: { item: ReviewItem; index: number; total: number; draft: string; result?: ReviewResult; onDraft: (value: string) => void; onGrade: () => void; onNext: () => void; accuracy: number; attempted: number; reviewCount: number; today: number }) {
