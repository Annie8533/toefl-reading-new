...
type StoredProgress = { attempted: number; correct: number; wrongByGap: Record<string, number>; today: string; todayGroups: number; passageCursor: number };
...
const blankProgress = (): StoredProgress => ({ attempted: 0, correct: 0, wrongByGap: {}, today: todayKey(), todayGroups: 0, passageCursor: 0 });
...
export default function Home() {
  ...
  const newRound = () => { const next = screen === "summary" ? seed : (seed + session.length) % bank.length; setSeed(next); setProgress(previous => ({ ...previous, passageCursor: next })); setMode("exam"); setScreen("questions"); setSession(makeRound(progress, next)); setActive(0); setAnswers({}); setResults({}); setReviewItems([]); setReviewActive(0); setReviewDraft({}); setReviewResults({}); setReviewLetterAlert(""); setRoundCounted(false); };
  ...
  const advanceExamPassage = () => { const nextCursor = (seed + 1) % bank.length; setSeed(nextCursor); setProgress(previous => ({ ...previous, passageCursor: nextCursor })); if (active < session.length - 1) { setActive(index => index + 1); return; } if (!roundCounted) { setRoundCounted(true); setProgress(previous => ({ ...previous, passageCursor: nextCursor, today: todayKey(), todayGroups: previous.today === todayKey() ? previous.todayGroups + 1 : 1 })); } setScreen("summary"); };
