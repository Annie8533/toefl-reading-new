...
type StoredProgress = { attempted: number; correct: number; wrongByGap: Record<string, number>; today: string; todayGroups: number; dailyDone: boolean; pendingDailyReminderAt: number; passageCursor: number; priorityReviewTaskIds: number[]; newSincePriority: number };
...
const blankProgress = (): StoredProgress => ({ attempted: 0, correct: 0, wrongByGap: {}, today: todayKey(), todayGroups: 0, dailyDone: false, pendingDailyReminderAt: 0, passageCursor: 0, priorityReviewTaskIds: [], newSincePriority: 0 });
const readProgress = (): StoredProgress => { try { const saved = JSON.parse(localStorage.getItem(STORAGE_KEY) ?? "{}"); const state = { ...blankProgress(), ...saved, pendingDailyReminderAt: saved.pendingDailyReminderAt ?? 0, wrongByGap: saved.wrongByGap ?? {}, priorityReviewTaskIds: saved.priorityReviewTaskIds ?? [] }; return state.today === todayKey() ? state : { ...state, today: todayKey(), todayGroups: 0, dailyDone: false, pendingDailyReminderAt: 0 }; } catch { return blankProgress(); } };
...
export default function Home() {
  ...
  const finishDailyGoal = () => { setDailyFinished(true); setShowDailyReminder(false); setProgress(previous => ({ ...previous, dailyDone: true, pendingDailyReminderAt: 0 })); };
  ...
  return <div className="signal-app two-stage-app">
    <header className="signal-header"><a href="#top" className="signal-brand"><SignalMark/><span><b>TOEFL</b> // WORD<span>LAB</span></span></a><div className="header-meta"><span><Flame size={15}/> {mode === "review" ? "MISTAKE SPELLING REVIEW" : "PARAGRAPH EXAM MODE"}</span><span className="header-divider"/><b>{progress.todayGroups < 2 ? `DAY ${String(progress.todayGroups).padStart(2, "0")} / 02` : `DAY ${String(progress.todayGroups).padStart(2, "0")} · NEXT ${String(nextDailyCheckpoint(progress.todayGroups)).padStart(2, "0")}`}</b></div><button onClick={newRound} className="signal-button ghost"><RefreshCw size={16}/> NEW ROUND</button></header>
