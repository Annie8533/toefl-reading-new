...
export default function Home() {
  ...
  return <div className="signal-app two-stage-app">
    <header className="signal-header"><a href="#top" className="signal-brand"><SignalMark/><span><b>TOEFL</b> // WORD<span>LAB</span></span></a><div className="header-meta"><span><Flame size={15}/> {screen === "daily-choice" ? "DAILY WRAP" : mode === "review" ? "MISTAKE SPELLING REVIEW" : "PARAGRAPH EXAM MODE"}</span><span className="header-divider"/><b>{progress.todayGroups < 2 ? `DAY ${String(progress.todayGroups).padStart(2, "0")} / 02` : `DAY ${String(progress.todayGroups).padStart(2, "0")} · NEXT ${String(nextDailyCheckpoint(progress.todayGroups)).padStart(2, "0")}`}</b></div><button onClick={newRound} disabled={dailyFinished} className="signal-button ghost"><RefreshCw size={16}/> {dailyFinished ? "TODAY CLOSED" : "NEW ROUND"}</button></header>
...
function DailyWrapUp({ groups, wrongTotal, accuracy, highErrorCount, topTasks, nextGroup, priorityCount }: { groups: number; wrongTotal: number; accuracy: number; highErrorCount: number; topTasks: { task?: Task; wrong: number }[]; nextGroup: number; priorityCount: number }) { return <section className="daily-wrap"><div className="daily-wrap-hero"><div><p className="signal-kicker">DAILY REVIEW COMPLETE</p><h2>今天的練習 <em>已收束。</em></h2><p>今天不再載入新題。明天開啟時，會從下一個新題組繼續；今日錯題權重與待複習題組都會保留。</p></div><div className="daily-wrap-signal"><strong>{String(groups).padStart(2, "0")}</strong><span>GROUPS COMPLETED</span></div></div><div className="daily-wrap-grid"><div className="daily-wrap-stat"><span>今日完成組數</span><b>{groups}</b><small>GROUPS</small></div><div className="daily-wrap-stat"><span>今日段落錯空</span><b>{wrongTotal}</b><small>GAPS</small></div><div className="daily-wrap-stat"><span>今日段落正確率</span><b>{accuracy}%</b><small>ACCURACY</small></div><div className="daily-wrap-stat"><span>高錯待複習題組</span><b>{highErrorCount}</b><small>TODAY</small></div></div><div className="daily-wrap-plan"><div><p className="signal-kicker">TOMORROW&apos;S QUEUE</p><h3>下一次從題組 <em>{String(nextGroup).padStart(2, "0")}</em> 接續。</h3><p>{priorityCount > 0 ? `目前保留 ${priorityCount} 組高錯題；之後每完成 3 組新題，系統會插入 1 組複習。` : "今天沒有列入高錯清單的題組；一般錯字權重仍會保留，供後續複習排序使用。"}</p></div><div className="daily-wrap-gridmark" aria-hidden="true"><i>_</i><i>A</i><i>_</i><i>R</i><i>_</i><i>G</i></div></div><div className="daily-wrap-errors"><div><p className="signal-kicker">TODAY&apos;S ERROR SIGNAL</p><h3>今天需要回看的題組</h3></div>{topTasks.length ? <div className="daily-task-list">{topTasks.map(({ task, wrong }) => <div key={task?.task_id ?? wrong} className="daily-task"><b>{String(task ? bank.findIndex(item => item.task_id === task.task_id) + 1 : 0).padStart(2, "0")}</b><span>{task?.topic_anchor ?? "題組資料"}</span><em>{wrong} 個錯空</em></div>)}</div> : <p className="daily-wrap-empty">今天的段落沒有錯空；明天會直接從下一組新題開始。</p>}</div><p className="daily-wrap-close">TODAY CLOSED · 隔日自動解除日結狀態並續作</p></section> }
