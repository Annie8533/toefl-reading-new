...
import { ArrowRight, BookOpenCheck, CheckCircle2, CircleHelp, ClipboardCheck, Flame, Gauge, RefreshCw, RotateCcw, Target, Trophy, XCircle } from "lucide-react";
...
type ReviewItem = { task: Task; gap: ReturnType<typeof gapsOf>[number]; submitted: string };
...
export default function Home() {
  ...
  const [isReviewRetry, setIsReviewRetry] = useState(false);
  ...
  const newRound = () => { const next = seed + 1; setSeed(next); setSession(makeSession(progress, next)); setActive(0); setAnswers({}); setResults({}); setCounted([]); setShowReview(false); setIsReviewRetry(false); };
  ...
  const grade = () => { if (!task) return; const correct: Record<number,boolean> = {}; gaps.forEach(g => correct[g.number] = (current[g.number] ?? "").toLowerCase() === g.expected.toLowerCase()); const score = Object.values(correct).filter(Boolean).length; const isSessionComplete = session.every(item => item.task_id === task.task_id || Boolean(results[item.task_id])); setResults(prev => ({...prev, [task.task_id]: {correct, score}})); if (!counted.includes(task.task_id)) { setCounted(prev => [...prev, task.task_id]); setProgress(prev => { const missed = gaps.length-score; const weights={...prev.wrongByTask}; if(missed) weights[task.task_id]=(weights[task.task_id]??0)+missed; else if(weights[task.task_id]) weights[task.task_id]=Math.max(0,weights[task.task_id]-2); const completedToday = isSessionComplete && !isReviewRetry; return {...prev,attempted:prev.attempted+gaps.length,correct:prev.correct+score,wrongByTask:weights,today:todayKey(),todayGroups:completedToday ? (prev.today===todayKey()?prev.todayGroups+1:1) : prev.todayGroups}; }); } if (isSessionComplete) setShowReview(true); };
  const reviewItems: ReviewItem[] = session.flatMap(item => { const itemResult = results[item.task_id]; const submitted = answers[item.task_id] ?? {}; if (!itemResult) return []; return gapsOf(item).filter(g => !itemResult.correct[g.number]).map(g => ({ task: item, gap: g, submitted: submitted[g.number] ?? "" })); });
  const retryMissed = () => { const tasks = Array.from(new Map(reviewItems.map(item => [item.task.task_id, item.task])).values()); if (!tasks.length) { newRound(); return; } setSession(tasks); setActive(0); setAnswers({}); setResults({}); setCounted([]); setShowReview(false); setIsReviewRetry(true); };
  if(!task) return null;
  ...
  if (showReview) return <ReviewScreen session={session} reviewItems={reviewItems} accuracy={accuracy} onRetry={retryMissed} onNextRound={newRound}/>;
...
function ReviewScreen({session,reviewItems,accuracy,onRetry,onNextRound}:{session:Task[];reviewItems:ReviewItem[];accuracy:number;onRetry:()=>void;onNextRound:()=>void}){ const correct = session.length * 10 - reviewItems.length; return <div className="signal-app review-app"><header className="signal-header"><a href="#top" className="signal-brand"><SignalMark/><span><b>TOEFL</b> // WORD<span>LAB</span></span></a><div className="header-meta"><span><BookOpenCheck size={15}/> REVIEW MODE</span><span className="header-divider"/><b>ROUND COMPLETE</b></div></header><main id="top" className="review-shell"><section className="review-hero"><div><p className="signal-kicker">ROUND COMPLETE · REVIEW REQUIRED</p><h1>Read the <em>misses.</em><br/>Recover the signal.</h1><p>這一輪已完成。先看錯在哪一個詞、錯在什麼拼字，再帶著完整段落脈絡重做。</p></div><div className="review-score"><Trophy size={27}/><strong>{accuracy}%</strong><span>ROUND ACCURACY</span><small>{correct} / {session.length * 10} GAPS CORRECT</small></div></section><section className="review-actions"><div><p className="signal-kicker">NEXT ACTION</p><h2>{reviewItems.length ? `${reviewItems.length} 個缺字等待回收` : "本輪全對，訊號乾淨。"}</h2></div><div className="review-buttons">{reviewItems.length > 0 && <button className="signal-button primary" onClick={onRetry}><RotateCcw size={17}/> RETRY MISSED</button>}<button className="signal-button next" onClick={onNextRound}><RefreshCw size={16}/> START NEXT ROUND</button></div></section><section className="review-list">{reviewItems.length ? reviewItems.map((item,index)=><article className="review-card" key={`${item.task.task_id}-${item.gap.number}`}><div className="review-index">{String(index+1).padStart(2,"0")}</div><div className="review-main"><p className="signal-kicker">SET {String(session.findIndex(x=>x.task_id===item.task.task_id)+1).padStart(2,"0")} · {item.task.topic_anchor}</p><p className="review-context">{item.task.full_passage}</p><div className="review-answer-grid"><span>YOUR INPUT <b>{item.gap.prefix}{item.submitted || "—"}</b></span><span>CORRECT WORD <b>{item.gap.answer}</b></span><span>ANSWER SLOT <b>#{item.gap.number}</b></span></div></div></article>) : <article className="review-card perfect"><CheckCircle2 size={30}/><div><p className="signal-kicker">CLEAN REVIEW</p><h2>所有 10 組題目均已正確完成。</h2><p>你可以直接開始下一輪，或讓系統從既有錯題權重安排下一批題組。</p></div></article>}</section></main><footer className="signal-footer"><span>REVIEW LOG / COMPLETE THE WORDS</span><span>{reviewItems.length} GAPS TO REVISIT</span><span>LOCAL BROWSER MEMORY</span></footer></div> }
