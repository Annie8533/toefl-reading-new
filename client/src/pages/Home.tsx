...
type ReviewResult = { correct: boolean; submitted: string; streak: number; completed: boolean };
...
export default function Home() {
  ...
  const [reviewStreak, setReviewStreak] = useState<Record<string, number>>({});
  ...
  const newRound = () => { const next = seed + 1; setSeed(next); setMode("exam"); setScreen("questions"); setSession(makeRound(progress, next)); setActive(0); setAnswers({}); setResults({}); setReviewItems([]); setReviewActive(0); setReviewDraft({}); setReviewResults({}); setReviewStreak({}); setRoundCounted(false); };
  ...
  const startTaskReview = () => {
    ...
    setMode("review"); setScreen("questions"); setReviewItems(misses); setReviewActive(0); setReviewDraft({}); setReviewResults({}); setReviewStreak({});
  };
  ...
  const gradeReview = () => {
    ...
    const streak = correct ? (reviewStreak[review.id] ?? 0) + 1 : 0;
    const completed = streak >= 10;
    setReviewStreak(previous => ({ ...previous, [review.id]: streak }));
    setReviewResults(previous => ({ ...previous, [review.id]: { correct, submitted, streak, completed } }));
    setReviewDraft(previous => ({ ...previous, [review.id]: "" }));
    setProgress(previous => { const weights = { ...previous.wrongByGap }; if (correct) weights[review.id] = completed ? Math.max(0, (weights[review.id] ?? 0) - 1) : (weights[review.id] ?? 0); else weights[review.id] = (weights[review.id] ?? 0) + 1; return { ...previous, attempted: previous.attempted + 1, correct: previous.correct + (correct ? 1 : 0), wrongByGap: weights }; });
    if (completed) window.setTimeout(() => nextReview(), 360);
  };
  const nextReview = () => { if (reviewActive < reviewItems.length - 1) { setReviewActive(index => index + 1); return; } setMode("exam"); setReviewItems([]); setReviewActive(0); setReviewDraft({}); setReviewResults({}); setReviewStreak({}); advanceExamPassage(); };
  ...
  return <div className="signal-app two-stage-app">
    ...
    <main id="top"><section className="signal-hero compact-hero"><img src="/manus-storage/cobalt-signal-hero_175d1730.png" alt="抽象字彙訊號網格"/><div className="hero-shade"/><div className="lexicon-grid" aria-hidden="true"><span>_</span><span>A</span><span>_</span><span>R</span><span>_</span><span>G</span></div><div className="hero-content"><p className="signal-kicker">TOEFL iBT 2026 · COMPLETE THE WORDS</p><h1>{mode === "review" ? <>Review the <em>word.</em></> : <>Complete the <em>passage.</em></>}</h1><p>{mode === "review" ? "錯題檢討時，逐題回到完整句子重新拼寫。" : "考試練習時，每題是一整段文字與十個字內填空。"}</p><div className="hero-readout"><strong>{mode === "review" ? reviewItems.length : "10"}</strong><span>{mode === "review" ? "MISSED WORDS / SINGLE-SENTENCE REVIEW" : "GAPS / ONE ACADEMIC PASSAGE"}</span><i/></div></div><div className="signal-orbit"><span/><span/><b>{screen === "summary" ? "OK" : String(mode === "review" ? reviewActive + 1 : active + 1).padStart(2, "0")}</b></div></section>
      <section className="signal-shell">{screen === "summary" ? <Summary mode="exam" score={examScore} total={session.length * 10} hasMisses={false} onReview={newRound} onNext={newRound}/> : mode === "exam" && task ? <ExamStage task={task} gaps={gaps} active={active} session={session} current={current} result={taskResult} renderPassage={renderPassage} onGrade={gradeTask} onNext={continueAfterExam} onSelect={setActive} accuracy={accuracy} attempted={progress.attempted} reviewCount={reviewCount} today={progress.todayGroups}/> : review ? <TenSpellReviewStage item={review} index={reviewActive} total={reviewItems.length} draft={reviewDraft[review.id] ?? ""} result={reviewResult} streak={reviewStreak[review.id] ?? 0} onDraft={(value: string) => setReviewDraft(previous => ({ ...previous, [review.id]: normalized(value, review.gap.answer, review.gap.expected.length) }))} onGrade={gradeReview} accuracy={accuracy} attempted={progress.attempted} reviewCount={reviewCount} today={progress.todayGroups}/> : null}<div className="signal-bottom"><div className="feature-block blue"><img src="/manus-storage/cobalt-signal-quiz_5a1d2f55.png" alt="抽象學習信號"/><div><p className="signal-kicker">ONE RULE</p><h3>Read wide.<br/><em>Spell precise.</em></h3></div></div><div className="feature-block white"><Gauge size={30}/><p className="signal-kicker">METHOD</p><h3>考試讀一整段，<br/>檢討練一個單字。</h3><span>LOCAL PROGRESS · NO ACCOUNT REQUIRED</span></div></div></section></main><footer className="signal-footer"><span>WORDLAB / COMPLETE THE WORDS</span><span>84 PASSAGES · 521 TARGET WORDS</span><span>LOCAL BROWSER MEMORY</span></footer>
  </div>;
}
...
function TenSpellReviewStage({ item, index, total, draft, result, streak, onDraft, onGrade, accuracy, attempted, reviewCount, today }: { item: ReviewItem; index: number; total: number; draft: string; result?: ReviewResult; streak: number; onDraft: (value: string) => void; onGrade: () => void; accuracy: number; attempted: number; reviewCount: number; today: number }) {
  const inputRef = useRef<HTMLInputElement>(null);
  useEffect(() => { if (!result?.completed) inputRef.current?.focus(); }, [item.id, streak, result?.completed]);
  const message = result ? (result.completed ? "此字已連續答對 10 次，正在進入下一錯詞。" : result.correct ? `正確。連續拼寫 ${streak}/10，請直接再按 Enter 繼續。` : "拼寫不正確，連續計數已歸零。請直接重新輸入。") : "每個錯字須連續拼寫正確 10 次；答錯會歸零。";
  return <><div className="dashboard-bar"><div><p className="signal-kicker">MISTAKE SPELLING REVIEW · 10X MASTER</p><h2>Review <span>{String(index + 1).padStart(2, "0")}</span> <small>· {index + 1}/{total} missed words</small></h2></div><div className="session-tabs">{Array.from({ length: total }, (_, number) => <button key={number} className={number === index ? "is-active" : ""} aria-label={`錯題 ${number + 1}`}>{String(number + 1).padStart(2, "0")}</button>)}</div></div><div className="signal-workspace single-workspace"><section className="question-stage review-stage"><div className="stage-topline"><div><span className="stage-number">{String(index + 1).padStart(2, "0")}</span><span className="stage-topic">{item.task.topic_anchor}</span></div><span className={`stage-state ${result?.completed ? "ready" : ""}`}>{result?.completed ? "MASTERED 10/10" : `STREAK ${streak}/10`}</span></div><div className="instruction"><Target size={17}/><span>檢討模式：回到完整句子，連續正確拼寫同一個錯字 10 次。</span></div><p className="single-sentence"><span>{item.before}</span><span className="signal-word"><span className="word-prefix">{item.gap.prefix}</span><span className="signal-gap-wrap"><input ref={inputRef} value={draft} maxLength={item.gap.expected.length} spellCheck={false} autoCapitalize="none" disabled={Boolean(result?.completed)} onChange={event => onDraft(event.target.value)} onKeyDown={event => { if (event.key === "Enter") { event.preventDefault(); onGrade(); } }} style={{ width: `${Math.max(46, item.gap.expected.length * 18 + 20)}px` }} className={`signal-gap ${result ? (result.correct ? "ok" : "no") : ""} ${item.gap.answer === item.gap.answer.toUpperCase() ? "is-caps" : ""}`} aria-label={`錯題 ${index + 1}，字首 ${item.gap.prefix}`}/>{result && !result.correct && <span className="correction">{item.gap.expected}</span>}</span></span><span>{item.after}</span></p><div className="streak-meter"><span>10X SPELLING STREAK</span><strong>{streak}/10</strong><div className="streak-dots">{Array.from({ length: 10 }, (_, dot) => <i key={dot} className={dot < streak ? "is-filled" : ""}/>)}</div></div><div className="spelling-line"><span>TYPE THE MISSING LETTERS</span><b>{item.gap.prefix}</b><i>{"_".repeat(Math.max(3, item.gap.expected.length))}</i><small>ENTER TO CHECK · FOCUS STAYS HERE</small></div>{result && <div className={`grade-banner ${result.correct ? "win" : ""}`}>{result.correct ? <CheckCircle2 size={18}/> : <XCircle size={18}/>}<span>{message}</span></div>}<div className="stage-actions"><div className="input-hint"><CircleHelp size={15}/><span>NO MOUSE NEEDED · ENTER × 10 TO MASTER</span></div><div className="action-buttons"><button onClick={onGrade} disabled={Boolean(result?.completed)} className="signal-button primary"><ClipboardCheck size={17}/> CHECK SPELLING</button></div></div><div className="target-line"><span>REVIEW WORD</span><p>{item.gap.answer}</p></div></section><Metrics accuracy={accuracy} attempted={attempted} reviewCount={reviewCount} today={today}/></div></>;
}
