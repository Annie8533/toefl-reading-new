...
type StoredProgress = { attempted: number; correct: number; wrongByGap: Record<string, number>; today: string; todayGroups: number; passageCursor: number; priorityReviewTaskIds: number[]; newSincePriority: number };
...
const blankProgress = (): StoredProgress => ({ attempted: 0, correct: 0, wrongByGap: {}, today: todayKey(), todayGroups: 0, passageCursor: 0, priorityReviewTaskIds: [], newSincePriority: 0 });
const readProgress = (): StoredProgress => { try { const saved = JSON.parse(localStorage.getItem(STORAGE_KEY) ?? "{}"); const state = { ...blankProgress(), ...saved, wrongByGap: saved.wrongByGap ?? {}, priorityReviewTaskIds: saved.priorityReviewTaskIds ?? [] }; return state.today === todayKey() ? state : { ...state, today: todayKey(), todayGroups: 0 }; } catch { return blankProgress(); } };
